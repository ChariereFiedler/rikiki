var ht=Object.defineProperty;var er=Object.getOwnPropertyDescriptor;var ke=(s,t)=>()=>(s&&(t=s(s=0)),t);var ze=(s,t)=>{for(var e in t)ht(s,e,{get:t[e],enumerable:!0})};var n=(s,t,e,r)=>{for(var i=r>1?void 0:r?er(t,e):t,o=s.length-1,a;o>=0;o--)(a=s[o])&&(i=(r?a(t,e,i):a(i))||i);return r&&i&&ht(t,e,i),i};var rt={};ze(rt,{closeHelp:()=>Ct,toggleHelp:()=>br});function kr(s){let t=s.querySelector("#kb-overlay");if(t)return t;if(!s.querySelector(`style[${Mt}]`)){let i=document.createElement("style");i.setAttribute(Mt,"1"),i.textContent=fr,s.appendChild(i)}let e=document.createElement("div");return e.innerHTML=gr,t=e.firstElementChild,s.appendChild(t),t.addEventListener("click",()=>Ct(s.host)),t.querySelector(".kb-card")?.addEventListener("click",i=>i.stopPropagation()),t}function br(s){if(!s.shadowRoot)return;kr(s.shadowRoot).classList.toggle("open")}function Ct(s){s.shadowRoot?.querySelector("#kb-overlay")?.classList.remove("open")}var Mt,fr,gr,je=ke(()=>{"use strict";Mt="data-deck-help",fr=`
  #kb-overlay {
...
`});var Nt={};ze(Nt,{installPresenter:()=>wr});function Pt(s){let t=Array.from(s.children).filter(v=>v.tagName.toLowerCase().startsWith("deck-")),e=t.findIndex(v=>v.hasAttribute("active")),r=t[e]??null,i=t[e+1]??null,a=(r?.querySelector("deck-notes")?.textContent??"").trim(),h=document.querySelector('link[rel="stylesheet"][href*="rikiki"], link[rel="stylesheet"][href*="tokens"], link[rel="stylesheet"][href*="theme"]')?.href??"";return{current:e+1,total:t.length,slideHtml:r?.outerHTML??"",nextHtml:i?.outerHTML??null,notes:a,themeHref:h,bundleHref:yr}}function zt(s){Re&&Re.postMessage({type:"state",state:Pt(s)})}function wr(s){if(de=de??new WeakSet,de.has(s)){O?.close(),O=null;return}de.add(s),Re=new BroadcastChannel(Ht),s.addEventListener("slide-change",()=>zt(s)),Re.addEventListener("message",r=>{let i=r.data;i?.type==="key"&&i.key&&window.dispatchEvent(new KeyboardEvent("keydown",{key:i.key,shiftKey:!!i.shift,bubbles:!0})),i?.type==="hello"&&zt(s)});let t=Pt(s);if(O=window.open("","rikiki-presenter","width=1280,height=800,popup=yes"),!O){console.warn("[rikiki/presenter] popup was blocked \xB7 allow popups for this site"),de.delete(s);return}O.document.open(),O.document.write(xr(t)),O.document.close();let e=setInterval(()=>{O&&O.closed&&(clearInterval(e),de?.delete(s),O=null)},1e3)}var Ht,yr,O,Re,de,xr,it=ke(()=>{"use strict";Ht="rik-presenter",yr=new URL("./index.js",import.meta.url).href,O=null,Re=null,de=null;xr=s=>`<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
...
<script>
  const channel = new BroadcastChannel('${Ht}');
...
</html>`});var jt={};ze(jt,{mountOverview:()=>Hr});function Er(s){if(s.querySelector(`style[${It}]`))return;let t=document.createElement("style");t.setAttribute(It,"1"),t.textContent=_r,s.appendChild(t)}function $r(s,t){if(s.querySelector(`link[${Ot}]`))return;let e=document.createElement("link");e.rel="stylesheet",e.setAttribute(Ot,"1"),e.href=new URL("../tokens.css",t).href,s.appendChild(e)}function Ut(s){let e=s.slides[0]?.querySelector("h1");return e?Array.from(e.childNodes).map(r=>r.nodeName==="BR"?" ":r.textContent??"").join("").replace(/\s+/g," ").trim()||`Slide ${s.startIdx+1}`:`Slide ${s.startIdx+1}`}function Sr(s){return(s.textContent||"").replace(/\s+/g," ").trim().slice(0,400).toLowerCase()}function Lr(s,t){let e=new Map;if(s.querySelectorAll("[id]").forEach(i=>{e.set(i.id,i.id+t),i.id=i.id+t}),e.size===0)return;let r=i=>i.replace(/url\(['"]?#([^'")]+)['"]?\)/g,(o,a)=>e.has(a)?`url(#${e.get(a)})`:o);s.querySelectorAll("*").forEach(i=>{for(let o of Ar){let a=i.getAttribute(o);a&&a.includes("url(")&&i.setAttribute(o,r(a))}for(let o of["href","xlink:href"]){let a=i.getAttribute(o);a&&a.startsWith("#")&&e.has(a.slice(1))&&i.setAttribute(o,"#"+e.get(a.slice(1)))}})}function Tr(s,t){let e=s.querySelectorAll("svg");t.querySelectorAll("svg").forEach((r,i)=>{let o=e[i]?.getBoundingClientRect();if(o&&o.width>0)r.setAttribute("width",String(Math.round(o.width))),r.setAttribute("height",String(Math.round(o.height))),r.style.maxWidth="100%";else if(r.hasAttribute("viewBox")&&!r.hasAttribute("width")){let a=(r.getAttribute("viewBox")??"").split(/[\s,]+/).map(Number);a.length===4&&a[2]>0&&(r.setAttribute("width",String(a[2])),r.setAttribute("height",String(a[3])),r.style.maxWidth="100%",r.style.height="auto")}})}function Mr(s,t){let e=s.querySelectorAll("deck-mermaid");t.querySelectorAll("deck-mermaid").forEach((r,i)=>{let o=e[i],a=document.createElement("div");a.className="ov-mermaid-snap",a.innerHTML=o?.renderedSvg??"";let d=o?.getBoundingClientRect();d&&d.width>0&&(a.style.width=d.width+"px",a.style.height=d.height+"px"),r.replaceWith(a)})}function Cr(s,t){let e=s.cloneNode(!0);return e.setAttribute("active",""),Tr(s,e),Mr(s,e),Lr(e,`-ov${t}`),e}async function zr(s,t){let e=Array.from(t.querySelectorAll("deck-mermaid")).map(i=>i.whenRendered).filter(i=>!!i);e.length&&await Promise.all(e).catch(()=>{});let r=document.createElement("div");r.className="ov-thumb",r.appendChild(Cr(t,Number(s.dataset.idx))),s.insertBefore(r,s.firstChild),s.dataset.loaded="1",delete s.dataset.building}function Hr(s,t){let e=s.shadowRoot;if(!e)return()=>{};Er(e),$r(e,import.meta.url);let r=e.querySelector("#overview-grid");r||(r=document.createElement("div"),r.id="overview-grid",e.appendChild(r)),r.innerHTML="";let i=t.slides.length,o=i>60,a=i>300?140:i>150?160:i>60?180:220;r.style.setProperty("--ov-cell-min",a+"px");let d=window.innerWidth,h=window.innerHeight;r.style.setProperty("--ov-thumb-w",`${d}px`),r.style.setProperty("--ov-thumb-h",`${h}px`),requestAnimationFrame(()=>{let k=r.querySelector(".ov-cell")?.clientWidth??a;r.style.setProperty("--overview-scale",String(k/d))});let v=document.createElement("div");v.className="ov-bar";let g=document.createElement("input");g.className="ov-search",g.type="search",g.placeholder=`Search ${i} slides \xB7 type to filter`,g.spellcheck=!1,v.appendChild(g);let f=document.createElement("span");f.className="ov-count",f.textContent=`${i} slides \xB7 ${t.chapters.length} chapters`,v.appendChild(f);let x=document.createElement("span");x.className="ov-hint",x.textContent="O \xB7 Esc \xB7 close",v.appendChild(x),r.appendChild(v);let S=document.createElement("div");S.className="ov-body"+(o?"":" compact"),r.appendChild(S);let T=o?document.createElement("aside"):null;T&&(T.className="ov-aside",S.appendChild(T));let J=document.createElement("div");J.className="ov-main"+(o?"":" ov-path"),S.appendChild(J);let Be=[],lt=[],dt=new WeakMap,qe=new IntersectionObserver(k=>{for(let A of k){if(!A.isIntersecting)continue;let b=A.target;if(b.dataset.loaded||b.dataset.building)continue;let z=dt.get(b);z&&(b.dataset.building="1",qe.unobserve(b),zr(b,z))}},{root:null,rootMargin:"300px 0px",threshold:0});t.chapters.forEach((k,A)=>{let b=document.createElement("section");if(b.className="ov-chapter",b.id=`ov-chapter-${A}`,o){let y=document.createElement("header");y.className="ov-chapter-head";let Z=document.createElement("span");Z.className="ov-chapter-num",Z.textContent=String(A+1).padStart(2,"0"),y.appendChild(Z);let H=document.createElement("span");H.className="ov-chapter-title",H.textContent=Ut(k),y.appendChild(H);let w=document.createElement("span");w.className="ov-chapter-count",w.textContent=`${k.slides.length} slide${k.slides.length>1?"s":""}`,y.appendChild(w),b.appendChild(y)}let z=document.createElement("div");if(z.className="ov-row",k.slides.forEach((y,Z)=>{if(!o&&Z>0){let pt=document.createElement("div");pt.className="ov-connector",z.appendChild(pt)}let H=k.startIdx+Z,w=document.createElement("div");w.className="ov-cell",w.dataset.idx=String(H),w.dataset.search=Sr(y),H===t.currentIdx&&(w.dataset.current="1"),dt.set(w,y),qe.observe(w);let Q=document.createElement("span");Q.className="ov-cell-label",Q.textContent=String(H+1),w.appendChild(Q),w.addEventListener("click",()=>t.onPick(H)),z.appendChild(w)}),b.appendChild(z),J.appendChild(b),Be.push(b),T){let y=document.createElement("button");y.type="button",y.className="ov-aside-item",t.currentIdx>=k.startIdx&&t.currentIdx<k.startIdx+k.slides.length&&(y.dataset.active="1");let H=document.createElement("span");H.className="ov-aside-num",H.textContent=String(A+1).padStart(2,"0"),y.appendChild(H);let w=document.createElement("span");w.className="ov-aside-text",w.textContent=Ut(k);let Q=document.createElement("div");Q.className="ov-aside-count",Q.textContent=`${k.slides.length} slide${k.slides.length>1?"s":""}`,w.appendChild(Q),y.appendChild(w),y.addEventListener("click",()=>{b.scrollIntoView({behavior:"smooth",block:"start"})}),T.appendChild(y),lt.push(y)}});let Ye=null;T&&(Ye=new IntersectionObserver(k=>{let A=k.filter(b=>b.isIntersecting).sort((b,z)=>b.boundingClientRect.top-z.boundingClientRect.top);if(A.length>0){let b=Be.indexOf(A[0].target);b>=0&&lt.forEach((z,y)=>{y===b?z.dataset.active="1":delete z.dataset.active})}},{root:J,rootMargin:"0px 0px -70% 0px",threshold:0}),Be.forEach(k=>Ye.observe(k)));let ct=r.querySelectorAll(".ov-cell");return g.addEventListener("input",()=>{let k=g.value.trim().toLowerCase();if(!k){ct.forEach(A=>delete A.dataset.filteredOut);return}ct.forEach(A=>{(A.dataset.search||"").includes(k)?delete A.dataset.filteredOut:A.dataset.filteredOut="1"})}),requestAnimationFrame(()=>{let k=r.querySelector(".ov-cell[data-current]");k&&k.scrollIntoView({block:"center"})}),()=>{Ye?.disconnect(),qe.disconnect(),r&&(r.innerHTML="")}}var It,Ot,_r,Ar,st=ke(()=>{"use strict";It="data-deck-overview",Ot="data-overview-tokens",_r=`
...
`;Ar=["fill","stroke","clip-path","mask","filter","marker-start","marker-mid","marker-end","style"]});var Bt={};ze(Bt,{installTransitions:()=>Or});function Or(s){if(s.__rkTransitions)return s.__rkTransitions;let t=s.shadowRoot;if(!t)return()=>{};let e=document.createElement("style");e.textContent=Nr,e.setAttribute("data-rik-transitions",""),t.appendChild(e);function r(d,h){if(!h)return 0;let v=d.compareDocumentPosition(h);return v&Node.DOCUMENT_POSITION_PRECEDING?1:v&Node.DOCUMENT_POSITION_FOLLOWING?-1:0}let i=d=>{if(s.__rkMorphActive)return;let h=d,v=h.detail.current,g=h.detail.previous;if(!v)return;let f=v.dataset.transition||s.transition||"fade",x=r(v,g);f==="slide"&&x<0?f="slide-right":f==="slide-right"&&x>0?f="slide":f==="slide-up"&&x<0?f="slide-down":f==="slide-down"&&x>0&&(f="slide-up");let S=`rk-enter-${f}`,T=`rk-exit-${f}`,J=L[f]??480;g&&g!==v&&(g.classList.remove(...Rt,...Ir,"rk-leaving"),g.style.display="flex",g.style.zIndex="1",g.offsetWidth,g.classList.add("rk-leaving",T),window.setTimeout(()=>{g.classList.remove("rk-leaving",T),g.style.display="",g.style.zIndex=""},J+40)),v.classList.remove(...Rt),v.style.zIndex="2",v.offsetWidth,v.classList.add(S),window.setTimeout(()=>{v.classList.remove(S),v.style.zIndex=""},J+40)};s.addEventListener("slide-change",i);let o=s.querySelector(":scope > [active]");o&&i(new CustomEvent("slide-change",{detail:{current:o,previous:null}}));let a=()=>{s.removeEventListener("slide-change",i),e.remove(),delete s.__rkTransitions};return s.__rkTransitions=a,a}var L,C,Pr,Nr,Rt,Ir,ot=ke(()=>{"use strict";L={slide:560,"slide-up":520,"slide-down":520,"slide-right":560,fade:480,zoom:520,flip:560},C="var(--rik-motion__ease-out, cubic-bezier(0.16, 1, 0.3, 1))",Pr="var(--rik-motion__ease-spring, cubic-bezier(0.5, 1.8, 0.3, 1))",Nr=`
  ...
  ::slotted([active].rk-enter-slide)        { animation: rk-slide-in       ${L.slide}ms ${C}    both; }
  ::slotted(.rk-leaving.rk-exit-slide)      { animation: rk-slide-out      ${L.slide}ms ${C}    both; }
  ::slotted([active].rk-enter-slide-right)  { animation: rk-slide-right-in ${L["slide-right"]}ms ${C} both; }
  ::slotted(.rk-leaving.rk-exit-slide-right){ animation: rk-slide-right-out ${L["slide-right"]}ms ${C} both; }
  ::slotted([active].rk-enter-slide-up)     { animation: rk-slide-up-in    ${L["slide-up"]}ms ${C} both; }
  ::slotted(.rk-leaving.rk-exit-slide-up)   { animation: rk-slide-up-out   ${L["slide-up"]}ms ${C} both; }
  ::slotted([active].rk-enter-slide-down)   { animation: rk-slide-down-in  ${L["slide-down"]}ms ${C} both; }
  ::slotted(.rk-leaving.rk-exit-slide-down) { animation: rk-slide-down-out ${L["slide-down"]}ms ${C} both; }
  ::slotted([active].rk-enter-fade)         { animation: rk-fade-in        ${L.fade}ms ${C} both; }
  ::slotted(.rk-leaving.rk-exit-fade)       { animation: rk-fade-out       ${L.fade}ms ${C} both; }
  ::slotted([active].rk-enter-zoom)         { animation: rk-zoom-in        ${L.zoom}ms ${Pr} both; }
  ::slotted(.rk-leaving.rk-exit-zoom)       { animation: rk-zoom-out       ${L.zoom}ms ${C} both; }
  ::slotted([active].rk-enter-flip)         { animation: rk-flip-in        ${L.flip}ms ${C} both; }
  ::slotted(.rk-leaving.rk-exit-flip)       { animation: rk-flip-out       ${L.flip}ms ${C} both; }

  @media (prefers-reduced-motion: reduce) {
    ::slotted([active][class*='rk-enter-']),
    ::slotted(.rk-leaving)                  { animation: none; }
  }
`,Rt=["rk-enter-slide","rk-enter-slide-up","rk-enter-slide-down","rk-enter-slide-right","rk-enter-fade","rk-enter-zoom","rk-enter-flip"],Ir=["rk-exit-slide","rk-exit-slide-up","rk-exit-slide-down","rk-exit-slide-right","rk-exit-fade","rk-exit-zoom","rk-exit-flip"]});var Jr={};function Zt(s,t="#0a0a0a"){N||(N=document.createElement("div"),N.style.cssText=`position:fixed;bottom:12px;left:12px;z-index:9999;padding:6px 12px;background:${t};color:#F7CB44;font:600 11px/1.4 monospace;border-radius:6px;letter-spacing:.08em;text-transform:uppercase;opacity:0;transition:opacity .2s;pointer-events:none;border:1px solid #F7CB44`,document.body.appendChild(N)),N.textContent=s,N.style.opacity="1",N._t&&clearTimeout(N._t),N._t=setTimeout(()=>{N&&(N.style.opacity="0")},1500)}async function Qt(s){try{let t=await fetch(s+"?_lr="+Date.now(),{method:"HEAD",cache:"no-store"}),e=t.headers.get("last-modified")??t.headers.get("etag")??t.headers.get("content-length");if(!e)return!1;let r=Gt.get(s);return Gt.set(s,e),r!==void 0&&r!==e}catch{return!1}}async function Gr(){for(;;){for(let s of Jt)if(await Qt(s)){Zt(`reload \xB7 ${s.split("/").pop()}`),await new Promise(t=>setTimeout(t,150)),location.reload();return}await new Promise(s=>setTimeout(s,800))}}var $,Kr,Vr,Jt,Gt,N,Dt=ke(()=>{"use strict";$=s=>new URL(s,import.meta.url).href,Kr=s=>{try{return new URL(s,location.href).origin===location.origin}catch{return!1}},Vr=Array.from(document.querySelectorAll('link[rel="stylesheet"]')).map(s=>s.href).filter(Kr),Jt=[...Vr,$("./index.js"),$("./deck-root.js"),$("./deck-cover.js"),$("./deck-section.js"),$("./deck-feature.js"),$("./deck-split.js"),$("./deck-feature-cards.js"),$("./deck-takeaway.js"),$("./deck-md.js"),$("./deck-code.js"),$("./deck-callout.js"),$("./deck-card.js"),$("./deck-mermaid.js"),$("./shared-styles.js"),$("./deck-stack.js"),$("./deck-grid.js"),$("./deck-punch.js"),location.pathname],Gt=new Map;(async()=>(await Promise.all(Jt.map(Qt)),Zt("livereload on"),Gr()))()});var He=globalThis,Pe=He.ShadowRoot&&(He.ShadyCSS===void 0||He.ShadyCSS.nativeShadow)&&"adoptedStyleSheets"in Document.prototype&&"replace"in CSSStyleSheet.prototype,We=Symbol(),mt=new WeakMap,be=class{constructor(t,e,r){if(this._$cssResult$=!0,r!==We)throw Error("CSSResult is not constructable. Use `unsafeCSS` or `css` instead.");this.cssText=t,this.t=e}get styleSheet(){let t=this.o,e=this.t;if(Pe&&t===void 0){let r=e!==void 0&&e.length===1;r&&(t=mt.get(e)),t===void 0&&((this.o=t=new CSSStyleSheet).replaceSync(this.cssText),r&&mt.set(e,t))}return t}toString(){return this.cssText}},vt=s=>new be(typeof s=="string"?s:s+"",void 0,We),m=(s,...t)=>{let e=s.length===1?s[0]:t.reduce((r,i,o)=>r+(a=>{if(a._$cssResult$===!0)return a.cssText;if(typeof a=="number")return a;throw Error("Value passed to 'css' function must be a 'css' function result: "+a+". Use 'unsafeCSS' to pass non-literal values, but take care to ensure page security.")})(i)+s[o+1],s[0]);return new be(e,s,We)},ut=(s,t)=>{if(Pe)s.adoptedStyleSheets=t.map(e=>e instanceof CSSStyleSheet?e:e.styleSheet);else for(let e of t){let r=document.createElement("style"),i=He.litNonce;i!==void 0&&r.setAttribute("nonce",i),r.textContent=e.cssText,s.appendChild(r)}},Xe=Pe?s=>s:s=>s instanceof CSSStyleSheet?(t=>{let e="";for(let r of t.cssRules)e+=r.cssText;return vt(e)})(s):s;var{is:tr,defineProperty:rr,getOwnPropertyDescriptor:ir,getOwnPropertyNames:sr,getOwnPropertySymbols:or,getPrototypeOf:ar}=Object,Ne=globalThis,ft=Ne.trustedTypes,nr=ft?ft.emptyScript:"",lr=Ne.reactiveElementPolyfillSupport,ye=(s,t)=>s,xe={toAttribute(s,t){switch(t){case Boolean:s=s?nr:null;break;case Object:case Array:s=s==null?s:JSON.stringify(s)}return s},fromAttribute(s,t){let e=s;switch(t){case Boolean:e=s!==null;break;case Number:e=s===null?null:Number(s);break;case Object:case Array:try{e=JSON.parse(s)}catch{e=null}}return e}},Ie=(s,t)=>!tr(s,t),gt={attribute:!0,type:String,converter:xe,reflect:!1,useDefault:!1,hasChanged:Ie};Symbol.metadata??=Symbol("metadata"),Ne.litPropertyMetadata??=new WeakMap;var B=class extends HTMLElement{static addInitializer(t){this._$Ei(),(this.l??=[]).push(t)}static get observedAttributes(){return this.finalize(),this._$Eh&&[...this._$Eh.keys()]}static createProperty(t,e=gt){if(e.state&&(e.attribute=!1),this._$Ei(),this.prototype.hasOwnProperty(t)&&((e=Object.create(e)).wrapped=!0),this.elementProperties.set(t,e),!e.noAccessor){let r=Symbol(),i=this.getPropertyDescriptor(t,r,e);i!==void 0&&rr(this.prototype,t,i)}}static getPropertyDescriptor(t,e,r){let{get:i,set:o}=ir(this.prototype,t)??{get(){return this[e]},set(a){this[e]=a}};return{get:i,set(a){let d=i?.call(this);o?.call(this,a),this.requestUpdate(t,d,r)},configurable:!0,enumerable:!0}}static getPropertyOptions(t){return this.elementProperties.get(t)??gt}static _$Ei(){if(this.hasOwnProperty(ye("elementProperties")))return;let t=ar(this);t.finalize(),t.l!==void 0&&(this.l=[...t.l]),this.elementProperties=new Map(t.elementProperties)}static finalize(){if(this.hasOwnProperty(ye("finalized")))return;if(this.finalized=!0,this._$Ei(),this.hasOwnProperty(ye("properties"))){let e=this.properties,r=[...sr(e),...or(e)];for(let i of r)this.createProperty(i,e[i])}let t=this[Symbol.metadata];if(t!==null){let e=litPropertyMetadata.get(t);if(e!==void 0)for(let[r,i]of e)this.elementProperties.set(r,i)}this._$Eh=new Map;for(let[e,r]of this.elementProperties){let i=this._$Eu(e,r);i!==void 0&&this._$Eh.set(i,e)}this.elementStyles=this.finalizeStyles(this.styles)}static finalizeStyles(t){let e=[];if(Array.isArray(t)){let r=new Set(t.flat(1/0).reverse());for(let i of r)e.unshift(Xe(i))}else t!==void 0&&e.push(Xe(t));return e}static _$Eu(t,e){let r=e.attribute;return r===!1?void 0:typeof r=="string"?r:typeof t=="string"?t.toLowerCase():void 0}constructor(){super(),this._$Ep=void 0,this.isUpdatePending=!1,this.hasUpdated=!1,this._$Em=null,this._$Ev()}_$Ev(){this._$ES=new Promise(t=>this.enableUpdating=t),this._$AL=new Map,this._$E_(),this.requestUpdate(),this.constructor.l?.forEach(t=>t(this))}addController(t){(this._$EO??=new Set).add(t),this.renderRoot!==void 0&&this.isConnected&&t.hostConnected?.()}removeController(t){this._$EO?.delete(t)}_$E_(){let t=new Map,e=this.constructor.elementProperties;for(let r of e.keys())this.hasOwnProperty(r)&&(t.set(r,this[r]),delete this[r]);t.size>0&&(this._$Ep=t)}createRenderRoot(){let t=this.shadowRoot??this.attachShadow(this.constructor.shadowRootOptions);return ut(t,this.constructor.elementStyles),t}connectedCallback(){this.renderRoot??=this.createRenderRoot(),this.enableUpdating(!0),this._$EO?.forEach(t=>t.hostConnected?.())}enableUpdating(t){}disconnectedCallback(){this._$EO?.forEach(t=>t.hostDisconnected?.())}attributeChangedCallback(t,e,r){this._$AK(t,r)}_$ET(t,e){let r=this.constructor.elementProperties.get(t),i=this.constructor._$Eu(t,r);if(i!==void 0&&r.reflect===!0){let o=(r.converter?.toAttribute!==void 0?r.converter:xe).toAttribute(e,r.type);this._$Em=t,o==null?this.removeAttribute(i):this.setAttribute(i,o),this._$Em=null}}_$AK(t,e){let r=this.constructor,i=r._$Eh.get(t);if(i!==void 0&&this._$Em!==i){let o=r.getPropertyOptions(i),a=typeof o.converter=="function"?{fromAttribute:o.converter}:o.converter?.fromAttribute!==void 0?o.converter:xe;this._$Em=i;let d=a.fromAttribute(e,o.type);this[i]=d??this._$Ej?.get(i)??d,this._$Em=null}}requestUpdate(t,e,r,i=!1,o){if(t!==void 0){let a=this.constructor;if(i===!1&&(o=this[t]),r??=a.getPropertyOptions(t),!((r.hasChanged??Ie)(o,e)||r.useDefault&&r.reflect&&o===this._$Ej?.get(t)&&!this.hasAttribute(a._$Eu(t,r))))return;this.C(t,e,r)}this.isUpdatePending===!1&&(this._$ES=this._$EP())}C(t,e,{useDefault:r,reflect:i,wrapped:o},a){r&&!(this._$Ej??=new Map).has(t)&&(this._$Ej.set(t,a??e??this[t]),o!==!0||a!==void 0)||(this._$AL.has(t)||(this.hasUpdated||r||(e=void 0),this._$AL.set(t,e)),i===!0&&this._$Em!==t&&(this._$Eq??=new Set).add(t))}async _$EP(){this.isUpdatePending=!0;try{await this._$ES}catch(e){Promise.reject(e)}let t=this.scheduleUpdate();return t!=null&&await t,!this.isUpdatePending}scheduleUpdate(){return this.performUpdate()}performUpdate(){if(!this.isUpdatePending)return;if(!this.hasUpdated){if(this.renderRoot??=this.createRenderRoot(),this._$Ep){for(let[i,o]of this._$Ep)this[i]=o;this._$Ep=void 0}let r=this.constructor.elementProperties;if(r.size>0)for(let[i,o]of r){let{wrapped:a}=o,d=this[i];a!==!0||this._$AL.has(i)||d===void 0||this.C(i,void 0,o,d)}}let t=!1,e=this._$AL;try{t=this.shouldUpdate(e),t?(this.willUpdate(e),this._$EO?.forEach(r=>r.hostUpdate?.()),this.update(e)):this._$EM()}catch(r){throw t=!1,this._$EM(),r}t&&this._$AE(e)}willUpdate(t){}_$AE(t){this._$EO?.forEach(e=>e.hostUpdated?.()),this.hasUpdated||(this.hasUpdated=!0,this.firstUpdated(t)),this.updated(t)}_$EM(){this._$AL=new Map,this.isUpdatePending=!1}get updateComplete(){return this.getUpdateComplete()}getUpdateComplete(){return this._$ES}shouldUpdate(t){return!0}update(t){this._$Eq&&=this._$Eq.forEach(e=>this._$ET(e,this[e])),this._$EM()}updated(t){}firstUpdated(t){}};B.elementStyles=[],B.shadowRootOptions={mode:"open"},B[ye("elementProperties")]=new Map,B[ye("finalized")]=new Map,lr?.({ReactiveElement:B}),(Ne.reactiveElementVersions??=[]).push("2.1.2");var Qe=globalThis,kt=s=>s,Oe=Qe.trustedTypes,bt=Oe?Oe.createPolicy("lit-html",{createHTML:s=>s}):void 0,$t="$lit$",K=`lit$${Math.random().toFixed(9).slice(2)}$`,St="?"+K,dr=`<${St}>`,te=document,_e=()=>te.createComment(""),Ee=s=>s===null||typeof s!="object"&&typeof s!="function",De=Array.isArray,cr=s=>De(s)||typeof s?.[Symbol.iterator]=="function",Fe=`[ 	
\f\r]`,we=/<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g,yt=/-->/g,xt=/>/g,D=RegExp(`>|${Fe}(?:([^\\s"'>=/]+)(${Fe}*=${Fe}*(?:[^ 	
\f\r"'\`<>=]|("|')|))|$)`,"g"),wt=/'/g,_t=/"/g,At=/^(?:script|style|textarea|title)$/i,et=s=>(t,...e)=>({_$litType$:s,strings:t,values:e}),c=et(1),ri=et(2),ii=et(3),re=Symbol.for("lit-noChange"),_=Symbol.for("lit-nothing"),Et=new WeakMap,ee=te.createTreeWalker(te,129);function Lt(s,t){if(!De(s)||!s.hasOwnProperty("raw"))throw Error("invalid template strings array");return bt!==void 0?bt.createHTML(t):t}var pr=(s,t)=>{let e=s.length-1,r=[],i,o=t===2?"<svg>":t===3?"<math>":"",a=we;for(let d=0;d<e;d++){let h=s[d],v,g,f=-1,x=0;for(;x<h.length&&(a.lastIndex=x,g=a.exec(h),g!==null);)x=a.lastIndex,a===we?g[1]==="!--"?a=yt:g[1]!==void 0?a=xt:g[2]!==void 0?(At.test(g[2])&&(i=RegExp("</"+g[2],"g")),a=D):g[3]!==void 0&&(a=D):a===D?g[0]===">"?(a=i??we,f=-1):g[1]===void 0?f=-2:(f=a.lastIndex-g[2].length,v=g[1],a=g[3]===void 0?D:g[3]==='"'?_t:wt):a===_t||a===wt?a=D:a===yt||a===xt?a=we:(a=D,i=void 0);let S=a===D&&s[d+1].startsWith("/>")?" ":"";o+=a===we?h+dr:f>=0?(r.push(v),h.slice(0,f)+$t+h.slice(f)+K+S):h+K+(f===-2?d:S)}return[Lt(s,o+(s[e]||"<?>")+(t===2?"</svg>":t===3?"</math>":"")),r]},$e=class s{constructor({strings:t,_$litType$:e},r){let i;this.parts=[];let o=0,a=0,d=t.length-1,h=this.parts,[v,g]=pr(t,e);if(this.el=s.createElement(v,r),ee.currentNode=this.el.content,e===2||e===3){let f=this.el.content.firstChild;f.replaceWith(...f.childNodes)}for(;(i=ee.nextNode())!==null&&h.length<d;){if(i.nodeType===1){if(i.hasAttributes())for(let f of i.getAttributeNames())if(f.endsWith($t)){let x=g[a++],S=i.getAttribute(f).split(K),T=/([.?@])?(.*)/.exec(x);h.push({type:1,index:o,name:T[2],strings:S,ctor:T[1]==="."?Ve:T[1]==="?"?Ge:T[1]==="@"?Je:le}),i.removeAttribute(f)}else f.startsWith(K)&&(h.push({type:6,index:o}),i.removeAttribute(f));if(At.test(i.tagName)){let f=i.textContent.split(K),x=f.length-1;if(x>0){i.textContent=Oe?Oe.emptyScript:"";for(let S=0;S<x;S++)i.append(f[S],_e()),ee.nextNode(),h.push({type:2,index:++o});i.append(f[x],_e())}}}else if(i.nodeType===8)if(i.data===St)h.push({type:2,index:o});else{let f=-1;for(;(f=i.data.indexOf(K,f+1))!==-1;)h.push({type:7,index:o}),f+=K.length-1}o++}}static createElement(t,e){let r=te.createElement("template");return r.innerHTML=t,r}};function ne(s,t,e=s,r){if(t===re)return t;let i=r!==void 0?e._$Co?.[r]:e._$Cl,o=Ee(t)?void 0:t._$litDirective$;return i?.constructor!==o&&(i?._$AO?.(!1),o===void 0?i=void 0:(i=new o(s),i._$AT(s,e,r)),r!==void 0?(e._$Co??=[])[r]=i:e._$Cl=i),i!==void 0&&(t=ne(s,i._$AS(s,t.values),i,r)),t}var Ke=class{constructor(t,e){this._$AV=[],this._$AN=void 0,this._$AD=t,this._$AM=e}get parentNode(){return this._$AM.parentNode}get _$AU(){return this._$AM._$AU}u(t){let{el:{content:e},parts:r}=this._$AD,i=(t?.creationScope??te).importNode(e,!0);ee.currentNode=i;let o=ee.nextNode(),a=0,d=0,h=r[0];for(;h!==void 0;){if(a===h.index){let v;h.type===2?v=new Se(o,o.nextSibling,this,t):h.type===1?v=new h.ctor(o,h.name,h.strings,this,t):h.type===6&&(v=new Ze(o,this,t)),this._$AV.push(v),h=r[++d]}a!==h?.index&&(o=ee.nextNode(),a++)}return ee.currentNode=te,i}p(t){let e=0;for(let r of this._$AV)r!==void 0&&(r.strings!==void 0?(r._$AI(t,r,e),e+=r.strings.length-2):r._$AI(t[e])),e++}},Se=class s{get _$AU(){return this._$AM?._$AU??this._$Cv}constructor(t,e,r,i){this.type=2,this._$AH=_,this._$AN=void 0,this._$AA=t,this._$AB=e,this._$AM=r,this.options=i,this._$Cv=i?.isConnected??!0}get parentNode(){let t=this._$AA.parentNode,e=this._$AM;return e!==void 0&&t?.nodeType===11&&(t=e.parentNode),t}get startNode(){return this._$AA}get endNode(){return this._$AB}_$AI(t,e=this){t=ne(this,t,e),Ee(t)?t===_||t==null||t===""?(this._$AH!==_&&this._$AR(),this._$AH=_):t!==this._$AH&&t!==re&&this._(t):t._$litType$!==void 0?this.$(t):t.nodeType!==void 0?this.T(t):cr(t)?this.k(t):this._(t)}O(t){return this._$AA.parentNode.insertBefore(t,this._$AB)}T(t){this._$AH!==t&&(this._$AR(),this._$AH=this.O(t))}_(t){this._$AH!==_&&Ee(this._$AH)?this._$AA.nextSibling.data=t:this.T(te.createTextNode(t)),this._$AH=t}$(t){let{values:e,_$litType$:r}=t,i=typeof r=="number"?this._$AC(t):(r.el===void 0&&(r.el=$e.createElement(Lt(r.h,r.h[0]),this.options)),r);if(this._$AH?._$AD===i)this._$AH.p(e);else{let o=new Ke(i,this),a=o.u(this.options);o.p(e),this.T(a),this._$AH=o}}_$AC(t){let e=Et.get(t.strings);return e===void 0&&Et.set(t.strings,e=new $e(t)),e}k(t){De(this._$AH)||(this._$AH=[],this._$AR());let e=this._$AH,r,i=0;for(let o of t)i===e.length?e.push(r=new s(this.O(_e()),this.O(_e()),this,this.options)):r=e[i],r._$AI(o),i++;i<e.length&&(this._$AR(r&&r._$AB.nextSibling,i),e.length=i)}_$AR(t=this._$AA.nextSibling,e){for(this._$AP?.(!1,!0,e);t!==this._$AB;){let r=kt(t).nextSibling;kt(t).remove(),t=r}}setConnected(t){this._$AM===void 0&&(this._$Cv=t,this._$AP?.(t))}},le=class{get tagName(){return this.element.tagName}get _$AU(){return this._$AM._$AU}constructor(t,e,r,i,o){this.type=1,this._$AH=_,this._$AN=void 0,this.element=t,this.name=e,this._$AM=i,this.options=o,r.length>2||r[0]!==""||r[1]!==""?(this._$AH=Array(r.length-1).fill(new String),this.strings=r):this._$AH=_}_$AI(t,e=this,r,i){let o=this.strings,a=!1;if(o===void 0)t=ne(this,t,e,0),a=!Ee(t)||t!==this._$AH&&t!==re,a&&(this._$AH=t);else{let d=t,h,v;for(t=o[0],h=0;h<o.length-1;h++)v=ne(this,d[r+h],e,h),v===re&&(v=this._$AH[h]),a||=!Ee(v)||v!==this._$AH[h],v===_?t=_:t!==_&&(t+=(v??"")+o[h+1]),this._$AH[h]=v}a&&!i&&this.j(t)}j(t){t===_?this.element.removeAttribute(this.name):this.element.setAttribute(this.name,t??"")}},Ve=class extends le{constructor(){super(...arguments),this.type=3}j(t){this.element[this.name]=t===_?void 0:t}},Ge=class extends le{constructor(){super(...arguments),this.type=4}j(t){this.element.toggleAttribute(this.name,!!t&&t!==_)}},Je=class extends le{constructor(t,e,r,i,o){super(t,e,r,i,o),this.type=5}_$AI(t,e=this){if((t=ne(this,t,e,0)??_)===re)return;let r=this._$AH,i=t===_&&r!==_||t.capture!==r.capture||t.once!==r.once||t.passive!==r.passive,o=t!==_&&(r===_||i);i&&this.element.removeEventListener(this.name,this,r),o&&this.element.addEventListener(this.name,this,t),this._$AH=t}handleEvent(t){typeof this._$AH=="function"?this._$AH.call(this.options?.host??this.element,t):this._$AH.handleEvent(t)}},Ze=class{constructor(t,e,r){this.element=t,this.type=6,this._$AN=void 0,this._$AM=e,this.options=r}get _$AU(){return this._$AM._$AU}_$AI(t){ne(this,t)}};var hr=Qe.litHtmlPolyfillSupport;hr?.($e,Se),(Qe.litHtmlVersions??=[]).push("3.3.3");var Tt=(s,t,e)=>{let r=e?.renderBefore??t,i=r._$litPart$;if(i===void 0){let o=e?.renderBefore??null;r._$litPart$=i=new Se(t.insertBefore(_e(),o),o,void 0,e??{})}return i._$AI(s),i};var tt=globalThis,p=class extends B{constructor(){super(...arguments),this.renderOptions={host:this},this._$Do=void 0}createRenderRoot(){let t=super.createRenderRoot();return this.renderOptions.renderBefore??=t.firstChild,t}update(t){let e=this.render();this.hasUpdated||(this.renderOptions.isConnected=this.isConnected),super.update(t),this._$Do=Tt(e,this.renderRoot,this.renderOptions)}connectedCallback(){super.connectedCallback(),this._$Do?.setConnected(!0)}disconnectedCallback(){super.disconnectedCallback(),this._$Do?.setConnected(!1)}render(){return re}};p._$litElement$=!0,p.finalized=!0,tt.litElementHydrateSupport?.({LitElement:p});var mr=tt.litElementPolyfillSupport;mr?.({LitElement:p});(tt.litElementVersions??=[]).push("4.2.2");var u=s=>(t,e)=>{e!==void 0?e.addInitializer(()=>{customElements.define(s,t)}):customElements.define(s,t)};var vr={attribute:!0,type:String,converter:xe,reflect:!1,hasChanged:Ie},ur=(s=vr,t,e)=>{let{kind:r,metadata:i}=e,o=globalThis.litPropertyMetadata.get(i);if(o===void 0&&globalThis.litPropertyMetadata.set(i,o=new Map),r==="setter"&&((s=Object.create(s)).wrapped=!0),o.set(e.name,s),r==="accessor"){let{name:a}=e;return{set(d){let h=t.get.call(this);t.set.call(this,d),this.requestUpdate(a,h,s,!0,d)},init(d){return d!==void 0&&this.C(a,void 0,s,d),d}}}if(r==="setter"){let{name:a}=e;return function(d){let h=this[a];t.call(this,d),this.requestUpdate(a,h,s,!0,d)}}throw Error("Unsupported decorator location: "+r)};function l(s){return(t,e)=>typeof e=="object"?ur(s,t,e):((r,i,o)=>{let a=i.hasOwnProperty(o);return i.constructor.createProperty(o,r),a?Object.getOwnPropertyDescriptor(i,o):void 0})(s,t,e)}function I(s){return l({...s,state:!0,attribute:!1})}var M=class extends p{constructor(){super(...arguments);this.current=0;this.step=0;this.blank=null;this.overview=!1;this.transition=null;this.autoplay=0;this.loop=!1;this.swipe=!1;this.mouseNav=null;this.slides=[];this.chapters=[];this._overviewTeardown=null;this._transitionLoaded=!1;this._autoplayTimer=null;this._autoplayPaused=!1;this._swipeStartX=0;this._swipeStartY=0;this._swipePointerId=null;this._navDownX=0;this._navDownY=0;this._wheelAccum=0;this._wheelLockUntil=0;this._onHoverEnter=()=>{this._autoplayPaused=!0,this._stopAutoplay()};this._onHoverLeave=()=>{this._autoplayPaused=!1,this.autoplay>0&&this._startAutoplay()};this._onPointerDown=e=>{!this.swipe||e.pointerType==="mouse"&&e.button!==0||e.target?.closest("a, button, input, textarea, [contenteditable]")||(this._swipePointerId=e.pointerId,this._swipeStartX=e.clientX,this._swipeStartY=e.clientY)};this._onPointerUp=e=>{if(this._swipePointerId===null||e.pointerId!==this._swipePointerId)return;let r=e.clientX-this._swipeStartX,i=e.clientY-this._swipeStartY;this._swipePointerId=null,!(Math.abs(r)<60||Math.abs(r)<Math.abs(i)*2)&&(this._stopAutoplay(),r<0?this._advance():this._back(),this.autoplay>0&&!this._autoplayPaused&&this._startAutoplay())};this._onNavPointerDown=e=>{this._navDownX=e.clientX,this._navDownY=e.clientY};this._onClickNav=e=>{if(!this._mouseEnabled("click")||this.overview||this.blank||this.shadowRoot?.querySelector("#kb-overlay.open")||Math.hypot(e.clientX-this._navDownX,e.clientY-this._navDownY)>5)return;let r=window.getSelection();r&&!r.isCollapsed||e.composedPath().some(o=>o instanceof HTMLElement?o.matches?.("a, button, input, textarea, select, [contenteditable], [data-no-advance]")?!0:o.id==="blank"||o.id==="kb-overlay"||o.id==="overview-grid"||o.id==="nav-arrows"||o.id==="kb-hint":!1)||(this.autoplay>0&&!this._autoplayPaused&&this._startAutoplay(),e.shiftKey?this._back():this._advance())};this._onWheel=e=>{if(!this._mouseEnabled("wheel")||this.overview||this.blank)return;e.preventDefault();let r=performance.now();if(r<this._wheelLockUntil||(this._wheelAccum+=e.deltaY,Math.abs(this._wheelAccum)<50))return;let i=this._wheelAccum>0;this._wheelAccum=0,this._wheelLockUntil=r+400,this.autoplay>0&&!this._autoplayPaused&&this._startAutoplay(),i?this._advance():this._back()};this._onAuxUp=e=>{this._mouseEnabled("aux")&&(e.button!==3&&e.button!==4||(e.preventDefault(),e.button===3?this._back():this._advance()))};this._onAuxClick=e=>{this._mouseEnabled("aux")&&(e.button===3||e.button===4)&&e.preventDefault()};this._onHash=()=>{this._readHash(!1)};this._onKey=e=>{if(!(e.target&&e.target.matches?.("input,textarea,[contenteditable]"))){if(this.autoplay>0&&!this._autoplayPaused&&this._startAutoplay(),this.overview){(e.key==="Escape"||e.key==="o"||e.key==="O"||e.key==="Enter")&&(e.preventDefault(),this.overview=!1);return}if(this.blank){e.preventDefault(),this.blank=null;return}if(e.key==="."||e.key==="b"||e.key==="B"){e.preventDefault(),this.blank="black";return}if(e.key===","||e.key==="w"||e.key==="W"){e.preventDefault(),this.blank="white";return}if(e.key==="?"||e.key==="h"||e.key==="H"){this._toggleHelp();return}if(e.key==="Escape"){this._closeHelp();return}if(e.key==="o"||e.key==="O"){e.preventDefault(),this.overview=!0;return}if(e.key==="p"||e.key==="P"){e.preventDefault(),this._togglePresenter();return}if(e.key==="Home"){this._goTo(0);return}if(e.key==="End"){this._goTo(this.slides.length-1);return}if(e.key===" "||e.key==="PageDown"){e.preventDefault(),this._advance();return}if(e.key==="PageUp"){e.preventDefault(),this._back();return}if(this._has2DNav()){let{c:r,i}=this._coords(this.current);if(e.key==="ArrowRight"){e.preventDefault(),r+1<this.chapters.length?this._goToCoords(r+1,0):this._advance();return}if(e.key==="ArrowLeft"){e.preventDefault(),r-1>=0?this._goToCoords(r-1,0):this._back();return}if(e.key==="ArrowDown"){e.preventDefault();let o=this.chapters[r];o&&i+1<o.slides.length?this._goToCoords(r,i+1):this._advance();return}if(e.key==="ArrowUp"){e.preventDefault(),i-1>=0?this._goToCoords(r,i-1):this._back();return}}else{if(e.key==="ArrowRight"||e.key==="ArrowDown"){e.preventDefault(),this._advance();return}if(e.key==="ArrowLeft"||e.key==="ArrowUp"){e.preventDefault(),this._back();return}}}}}firstUpdated(){this.slides=Array.from(this.querySelectorAll(":scope > *")).filter(e=>e.tagName?.toLowerCase().startsWith("deck-")&&e.tagName?.toLowerCase()!=="deck-root"),this._buildChapters(),this._readHash(!0),this._applyActive(),this._applyStep(),this._updateUI(),this.requestUpdate(),window.addEventListener("keydown",this._onKey),window.addEventListener("hashchange",this._onHash),this.addEventListener("pointerdown",this._onNavPointerDown),this.addEventListener("click",this._onClickNav),this.addEventListener("wheel",this._onWheel,{passive:!1}),window.addEventListener("mouseup",this._onAuxUp),window.addEventListener("auxclick",this._onAuxClick),this.autoplay>0&&this._startAutoplay(),this.swipe?(this.addEventListener("pointerdown",this._onPointerDown),this.addEventListener("pointerup",this._onPointerUp),this.addEventListener("pointercancel",this._onPointerUp),this.addEventListener("mouseenter",this._onHoverEnter),this.addEventListener("mouseleave",this._onHoverLeave)):this.autoplay>0&&(this.addEventListener("mouseenter",this._onHoverEnter),this.addEventListener("mouseleave",this._onHoverLeave))}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("keydown",this._onKey),window.removeEventListener("hashchange",this._onHash),this.removeEventListener("pointerdown",this._onNavPointerDown),this.removeEventListener("click",this._onClickNav),this.removeEventListener("wheel",this._onWheel),window.removeEventListener("mouseup",this._onAuxUp),window.removeEventListener("auxclick",this._onAuxClick),this._stopAutoplay(),this.removeEventListener("pointerdown",this._onPointerDown),this.removeEventListener("pointerup",this._onPointerUp),this.removeEventListener("pointercancel",this._onPointerUp),this.removeEventListener("mouseenter",this._onHoverEnter),this.removeEventListener("mouseleave",this._onHoverLeave)}_startAutoplay(){this._stopAutoplay(),!(this.autoplay<=0||this._autoplayPaused)&&(this._autoplayTimer=window.setInterval(()=>this._autoTick(),this.autoplay))}_stopAutoplay(){this._autoplayTimer!==null&&(window.clearInterval(this._autoplayTimer),this._autoplayTimer=null)}_autoTick(){if(this.overview)return;this.current>=this.slides.length-1&&this.step>=this._maxSteps()&&this.loop?this._goTo(0):this._advance()}_buildChapters(){this.chapters=[];let e=null;this.slides.forEach((r,i)=>{r.tagName?.toLowerCase()==="deck-section"||!e?(e={startIdx:i,slides:[r]},this.chapters.push(e)):e.slides.push(r)})}_has2DNav(){return this.chapters.some(e=>e.slides.length>1)&&this.chapters.length>1}_mouseEnabled(e){let r=(this.mouseNav??"all").trim();return r==="none"?!1:r===""||r==="all"?!0:r.split(/\s+/).includes(e)}_coords(e){for(let r=0;r<this.chapters.length;r++){let i=this.chapters[r],o=e-i.startIdx;if(o>=0&&o<i.slides.length)return{c:r,i:o}}return{c:0,i:0}}_flatFromCoords(e,r){let i=this.chapters[e];return i?i.startIdx+Math.max(0,Math.min(i.slides.length-1,r)):0}_readHash(e){let r=location.hash,i=r.match(/^#(\d+)\.(\d+)(?:s(\d+))?$/),o=r.match(/^#(\d+)(?:\.(\d+))?$/),a=this.current,d=this.step;if(this._has2DNav()&&i){let h=parseInt(i[1],10)-1,v=parseInt(i[2],10)-1;a=this._flatFromCoords(Math.max(0,h),Math.max(0,v)),d=i[3]?parseInt(i[3],10):0}else if(o)a=parseInt(o[1],10)-1,d=o[2]?parseInt(o[2],10):0;else return;a===this.current&&d===this.step&&!e||(this.current=Math.max(0,Math.min(this.slides.length-1,a)),this.step=Math.max(0,d),e||(this._applyActive(),this._applyStep(),this._updateUI()))}_writeHash(){let e=`#${this.current+1}`+(this.step>0?`.${this.step}`:"");if(location.hash!==e)try{history.replaceState(null,"",e)}catch{}}async _toggleHelp(){(await Promise.resolve().then(()=>(je(),rt))).toggleHelp(this)}async _togglePresenter(){(await Promise.resolve().then(()=>(it(),Nt))).installPresenter(this)}async _closeHelp(){(await Promise.resolve().then(()=>(je(),rt))).closeHelp(this)}async _renderOverviewIfActive(){if(!this.overview){this._overviewTeardown?.(),this._overviewTeardown=null;return}let{mountOverview:e}=await Promise.resolve().then(()=>(st(),jt));this._overviewTeardown=e(this,{slides:this.slides,chapters:this.chapters,currentIdx:this.current,onPick:r=>{this.overview=!1,this._goTo(r)}})}_maxSteps(){let e=this.slides[this.current];if(!e)return 0;let r=parseInt(e.getAttribute("steps")||e.dataset?.steps||"0",10);if(r>0)return r;let i=e.querySelector("deck-code[step-groups]");if(i)try{return JSON.parse(i.getAttribute("step-groups")).length}catch{}return 0}_advance(){let e=this._maxSteps();this.step<e?(this.step++,this._applyStep(),this._updateUI(),this._writeHash()):this.current<this.slides.length-1?this._goTo(this.current+1):this.loop&&this._goTo(0)}_back(){this.step>0?(this.step--,this._applyStep(),this._updateUI(),this._writeHash()):this.current>0?(this._goTo(this.current-1),this.step=this._maxSteps(),this._applyStep(),this._updateUI(),this._writeHash()):this.loop&&(this._goTo(this.slides.length-1),this.step=this._maxSteps(),this._applyStep(),this._updateUI(),this._writeHash())}_goTo(e){this.current=Math.max(0,Math.min(this.slides.length-1,e)),this.step=0,this._applyActive(),this._applyStep(),this._updateUI(),this._writeHash()}_goToCoords(e,r){let i=Math.max(0,Math.min(this.chapters.length-1,e)),o=this.chapters[i];if(!o)return;let a=Math.max(0,Math.min(o.slides.length-1,r));this._goTo(this._flatFromCoords(i,a))}_applyActive(){let e=this.slides.find(i=>i.hasAttribute("active"))??null,r=this.slides[this.current]??null;this.slides.forEach((i,o)=>{let a=o===this.current;i.toggleAttribute("active",a),a&&i.querySelectorAll("deck-mermaid").forEach(d=>d.render?.())}),this.transition&&!this._transitionLoaded&&(this._transitionLoaded=!0,Promise.resolve().then(()=>(ot(),Bt)).then(i=>i.installTransitions(this))),e!==r&&this.dispatchEvent(new CustomEvent("slide-change",{detail:{current:r,previous:e},bubbles:!1}))}_applyStep(){let e=this.slides[this.current];e&&(e.applyStep?.(this.step),e.querySelectorAll("*").forEach(r=>r.applyStep?.(this.step)),e.querySelectorAll("[data-step-block]").forEach(r=>{let i=parseInt(r.dataset.stepBlock,10);r.style.transition="opacity 0.25s ease",r.style.opacity=this.step===0||i<=this.step?"1":"0.15"}))}_updateUI(){let e=this.slides.length,r=this.current+1,i=this.renderRoot.querySelector("#progress"),o=this.renderRoot.querySelector("#counter"),a=this.renderRoot.querySelector("#step-dots");i&&(i.style.width=r/e*100+"%"),o&&(o.textContent=`${r} / ${e}`);let d=this._maxSteps();a&&(a.innerHTML=d===0?"":Array.from({length:d},(h,v)=>`<div class="dot${v<this.step?" active":""}"></div>`).join(""))}updated(){this._updateUI(),this._renderOverviewIfActive()}_navArrows(){if(!this._mouseEnabled("arrows")||this.overview)return"";let e=this.slides.length;if(e===0)return"";let r=this.current===0&&this.step===0,i=this.current>=e-1&&this.step>=this._maxSteps();if(this._has2DNav()){let{c:o,i:a}=this._coords(this.current),d=this.chapters[o];return c`<div id="nav-arrows"> <button class="nav-btn" title="Previous chapter" ?disabled=${!this.loop&&this.current===0} @click=${()=>o>0?this._goToCoords(o-1,0):this._back()}>&lsaquo;</button> <button class="nav-btn" title="Up" ?disabled=${a===0} @click=${()=>this._goToCoords(o,a-1)}>&uarr;</button> <button class="nav-btn" title="Down" ?disabled=${!d||a+1>=d.slides.length} @click=${()=>this._goToCoords(o,a+1)}>&darr;</button> <button class="nav-btn" title="Next chapter" ?disabled=${!this.loop&&i} @click=${()=>o+1<this.chapters.length?this._goToCoords(o+1,0):this._advance()}>&rsaquo;</button> </div>`}return c`<div id="nav-arrows"> <button class="nav-btn" title="Previous" ?disabled=${!this.loop&&r} @click=${()=>this._back()}>&lsaquo;</button> <button class="nav-btn" title="Next" ?disabled=${!this.loop&&i} @click=${()=>this._advance()}>&rsaquo;</button> </div>`}render(){return c`<div id="progress"></div> <div id="counter"></div> <div id="step-dots"></div> <div id="kb-hint"> <kbd>←</kbd><kbd>→</kbd> ${this._has2DNav()?c`<kbd>↑</kbd><kbd>↓</kbd>`:""}
        <span>·</span>
        <kbd>O</kbd>
        <span>·</span>
        <kbd>P</kbd>
    ...
    `}};q.styles=[...P,m`:host{justify-content:flex-start}.body{flex:1;min-height:0;display:grid;grid-template-columns:1fr 1fr;grid-template-rows:minmax(0,1fr);gap:var(--_gap,var(--deck-split-gap,var(--rik-space-5)))}:host([cols="1-2"]) .body{grid-template-columns:1fr 2fr}:host([cols="2-1"]) .body{grid-template-columns:2fr 1fr}:host([cols="3"]) .body{grid-template-columns:1fr 1fr 1fr;gap:var(--_gap,var(--deck-split-gap,var(--rik-space-4)))}.col{display:flex;flex-direction:column;min-height:0;min-width:0;gap:var(--_col-gap,var(--deck-split-col-gap,var(--rik-space-3)));overflow:hidden}.col.center{justify-content:center}`],n([l({type:String})],q.prototype,"eyebrow",2),n([l({type:String})],q.prototype,"cols",2),n([l({type:String})],q.prototype,"gap",2),n([l({type:String,attribute:"col-gap"})],q.prototype,"colGap",2),q=n([u("deck-split")],q);var he=class extends p{render(){return c`${this.eyebrow?c`<span class="lbl">${this.eyebrow}</span>`:""}
      <slot name="title"></slot>
      <slot name="lead"></slot>
