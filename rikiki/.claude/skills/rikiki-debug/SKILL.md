...
| Zoom does nothing / "ça zoom pas" | Two layers: (1) ctrl/⌘+wheel & trackpad pinch are now left for the browser (older builds swallowed them via the wheel-nav `preventDefault` — rebuild). (2) In the default fixed canvas, zoom-to-fit always refits the deck to the viewport, so browser zoom can't magnify slide content and `vw`/`vh` track the window, not the canvas. For content that must really zoom (interactive embeds, zoomable diagrams) put that slide in `fluid` (per-slide `fluid` attribute) or the whole deck in `fluid`, and size with `cqw/cqh` |
